...
class SaltStream(multiprocessing.Process):
    '''
    Object that all SaltStream IO modules should inherit.
    '''
    def __init__(self, opts):
        '''
        Pass in the module options from SaltStreamDaemon.start()
        '''
        self.opts = opts
        self.opts['stream'] = True
    ...
    def call(self, _fun, _arg):
        '''
        Call the module defined in the stream_config
        '''
        ret = {}
        fun = _fun
        ...
        try:
            args, kwargs = salt.minion.parse_args_and_kwargs(
                self.minion.functions[fun], _arg)
            sdata = {
                    'fun': fun,
                    'pid': os.getpid(),
                    'jid': ret['jid'],
